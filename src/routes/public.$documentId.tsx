...
export const Route = createFileRoute("/public/$documentId")({
	component: PublicDocument,
	pendingComponent: PublicDocumentSkeleton,
...
function PublicDocument() {
	...
	const publicUrl = `${origin}/public/${documentId}`;
	...
	if (!document) {
		return (
			<main className="min-h-svh w-full flex-1 flex flex-col bg-background">
				{showGuestBanner && (
					<>
						<PublicLoginBanner onLogIn={() => setLoginOpen(true)} />
				...
				<div className="flex flex-1 items-center justify-center px-6">
					<Empty className="w-full max-w-md">
						<EmptyHeader>
							...
							<EmptyTitle>This site isn&apos;t published</EmptyTitle>
							<EmptyDescription>
								Ask the owner to publish this page to make it available as a
								site.
							</EmptyDescription>
						</EmptyHeader>
					</Empty>
...
function PublicLoginBanner({ onLogIn }: { onLogIn: () => void }) {
	return (
		<div className="sticky top-0 z-50 w-full border-b bg-background/90 backdrop-blur">
			<div className="mx-auto flex max-w-6xl items-center justify-center gap-3 px-4 py-2 text-sm">
				<span className="text-muted-foreground">
					You&apos;re viewing this site as a guest.
				</span>
				<button
					type="button"
					onClick={onLogIn}
